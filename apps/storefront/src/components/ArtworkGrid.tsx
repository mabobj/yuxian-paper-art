  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-[18px]">
      {ARTWORKS.map((artwork) => (
        ...
        >
          <div className="relative overflow-hidden bg-brand-bg" style={{ height: "200px" }}>
            <Image
              src={`/design/red-window-art/${artwork.image}`}
              alt={artwork.name}
              fill
              className="object-cover group-hover:scale-105 transition-transform duration-500"
              sizes="(max-width: 640px) 50vw, (max-width: 1024px) 50vw, 275px"
            />
            {artwork.badge && (
              <span
            ...
            <span className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center rounded-full bg-white/80 z-10">
              <Image src="/design/red-window-art/vectors/icon-heart-outline.svg" alt="" width={16} height={16} className="w-4 h-4" />
            </span>
          </div>
          <div className="px-4 py-3" style={{ minHeight: "76px" }}>
